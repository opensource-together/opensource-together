...
export class FindProjectBySlugQuery implements IQuery {
  constructor(
...
@QueryHandler(FindProjectBySlugQuery)
export class FindProjectBySlugHandler
  implements IQueryHandler<FindProjectBySlugQuery>
{
  constructor(
  ...
  async execute(query: FindProjectBySlugQuery): Promise<
    Result<
      {
        author: {
          ownerId: string;
  ...
  > {
    ...
    const { slug, octokit } = query.props;
