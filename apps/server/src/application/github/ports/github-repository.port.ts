...
export interface GithubRepositoryPort {
  createGithubRepository(input: CreateGithubRepositoryInput, octokit: Octokit): Promise<Result<GithubRepositoryDto>>;
  inviteUserToRepository(input: InviteUserToRepoInput, octokit: Octokit): Promise<Result<GithubInvitationDto>>;
}
