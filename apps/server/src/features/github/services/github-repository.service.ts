...
import { GITHUB_REPOSITORY, RepositoryInfo } from "../repositories/github.repository.interface";
...
export class GithubRepositoryService {
  ...
  async createGithubRepository(input: CreateGithubRepositoryInput): Promise<Result<GithubRepositoryDto, string>> {
  ...
  async inviteUserToRepository(input: InviteUserToRepoInput): Promise<Result<GithubInvitationDto, string>> {
    const invitationResult = await this.githubRepository.inviteUserToRepository(input, input.octokit);
  ...
  async findRepositoryByOwnerAndName(ownerName: string, repoName: string, octokit: Octokit): Promise<Result<RepositoryInfo>> {
    const repositoryResult = await this.githubRepository.findRepositoryByOwnerAndName(
      ownerName,
      repoName,
      octokit,
    );
  ...
  async findCommitsByRepository(ownerName: string, repoName: string, octokit: Octokit): Promise<Result<any>> {
  ...
  async findContributorsByRepository(ownerName: string, repoName: string, octokit: Octokit): Promise<Result<any>> {
    const contributors = await this.githubRepository.findContributorsByRepository(
      ownerName,
      repoName,
      octokit
    );
  ...
  async findRepositoryOfAuthenticatedUser(octokit: Octokit): Promise<Result<any>> {
    const repositories = await this.githubRepository.findRepositoriesOfAuthenticatedUser(octokit);
  ...
  async getRepositoryReadme(ownerName: string, repoName: string, octokit: Octokit) {
