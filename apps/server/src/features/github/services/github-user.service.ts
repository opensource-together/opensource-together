...
export class GithubRepositoryService {
  ...
  async getUserTotalStars(octokit: Octokit): Promise<Result<number, string>> {
    ...
    if(!starsResult.success) {
      return Result.fail(
        `Erreur lors de la récupération des stars d'un utilisateur: ${starsResult}`
      );
    }
  ...
  async getUserContributedRepos(octokit: Octokit): Promise<Result<number, string>> {
    const contributedReposResult = await this.githubRepository.getUserContributedRepos(octokit);

    if(!contributedReposResult.success) {
      return Result.fail(
        `Erreur lors de la récupération des contributions d'un utilisateur: ${contributedReposResult}`
      );
    }
  ...
  async getUserCommitsLastYear(octokit: Octokit): Promise<Result<number, string>> {
    const commitsResult = await this.githubRepository.getUserCommitsLastYear(octokit);

    if(!commitsResult.success) {
      return Result.fail(
        `Erreur lors de la récupération des commits annuels d'un utilisateur: ${commitsResult}`
      );
    }
