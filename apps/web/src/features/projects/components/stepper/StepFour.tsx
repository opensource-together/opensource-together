...
export default function StepFour({ onBack }: { onBack: () => void }) {
  ...
  const [modalBlurState, setModalBlurState] = useState<'enter' | 'enter-active' | 'exit' | 'exit-active' | null>(null);
  ...
  const RoleModal = () => (
    ...
    >
      <div className="relative bg-white w-[603px] min-h-[400px] max-h-[90vh] rounded-[20px] border border-black/5 flex flex-col items-center px-6 py-6 shadow-xl font-geist overflow-y-auto">
        ...
        <div className="w-full flex items-center justify-center relative mb-8" style={{height:'32px'}}>
          <span className="absolute left-0" style={{width:'32px'}}></span>
          <span className="text-black/20 text-[12px] font-geist font-medium ">Create New Role</span>
          <button
        ...
        <div className="w-full flex flex-col gap-8">
          <div>
            <label className="block text-black font-geist font-medium text-[18px] mb-5">Choose Role Title</label>
            <div className="grid grid-cols-3 gap-3 ">
              {Array(6).fill('Back-end Developer').map((role, i) => (
                <button
                  key={i}
          ...
          <div>
            <label className="block text-black font-geist font-medium text-[18px] mb-5">Add skill set</label>
            <div className="flex flex-wrap gap-2">
              {Array(4).fill('Front-end Developer').map((skill, i) => (
                <span
                  key={i}
              ...
              {Array(4).fill('UX Designer').map((skill, i) => (
                <span
                  key={i}
              ...
              {Array(4).fill('MongoDB').map((skill, i) => (
                <span
                  key={i}
          ...
          <div className="border-t border-dashed border-black/10 my-2"></div>
          <div>
            <label className="block text-black font-geist font-medium text-[18px] mb-5">Add Description</label>
            <div className="relative mb-3">
              <textarea className="w-full h-[153px] rounded-[10px] border border-black/10 bg-white px-4 py-3 text-[15px] font-geist text-black/70 focus:outline-none resize-none" maxLength={250} placeholder="Describe new role" />
              <span className="absolute bottom-3 right-4 text-[13px] text-black/20">250/250</span>
            </div>
          </div>
          <Button
          width="full"
      ...
      <p className="text-[15px] text-center text-black/70 mb-8">
        ...
        <div className="relative">
          ...
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-black/20">
            {/* Icone de loupe */}
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7" stroke="currentColor" strokeWidth="2"/><path d="M20 20l-3-3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
          </span>
        </div>
      </div>
